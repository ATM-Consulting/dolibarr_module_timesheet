...
function get_ndfp(fk_user, fk_task, fk_timesheet) {
	...
	$.get('<?php echo dol_buildpath('/timesheet/script/interface.php',2); ?>?get=get_ndfp&fk_user='+fk_user+'&fk_task='+fk_task+'&fk_timesheet='+fk_timesheet,function(fk_ndfp){
		$.get('<?php echo dol_buildpath('/timesheet/script/interface.php',2); ?>?get=get_line_ndfp&fk_ndfp='+fk_ndfp,function(content){
			$("#viewlines").append(content);
		});
		pop_ndfp(fk_ndfp);
	});
...
function pop_ndfp(fk_ndfp){
	...
	$('#saisie').show(function(){
		$("#adlines").load('<?php echo dol_buildpath('/ndfp/ndfp.php',2) ?>?id='+fk_ndfp+' table[id=tablelines]',function() {
			
			$('#adlines form[id=addexpense]').submit(function() {
